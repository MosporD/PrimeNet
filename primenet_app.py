...
_configured_secret = (
    os.getenv('FLASK_SECRET_KEY_PRIMENET')
...
from modules.adjacency_gis import adjacency_gis_bp
...
from modules.rru_inventory import rru_inventory_bp
from modules.configuration_dashboard import configuration_dashboard_bp
...
app.register_blueprint(adjacency_gis_bp)
...
app.register_blueprint(rru_inventory_bp)
app.register_blueprint(configuration_dashboard_bp)
...
def _open_dashboard_browser():
    ...
    if _env_true('NCM_CONTAINER') or _env_true('NCM_DISABLE_AUTO_BROWSER'):
        return
    port = int(os.getenv('FLASK_PORT', os.getenv('PRIMENET_PORT', '8001')))
...
def health_live():
    ...
    """
    return jsonify({'status': 'ok', 'service': app.config.get('SERVICE_NAME', 'primenet')}), 200
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("PrimeNet — Engineering Portal")
    print("=" * 60)
