...
function renderNEs(nes) {
    ...
    list.innerHTML = nes.map(ne => `
        <div class="ne-item ${currentNE === ne.ne_name ? 'active' : ''}" onclick="selectNE('${ne.ne_name}')">
            <span class="ne-name">${ne.ne_name}</span>
            <span class="ne-meta">
                ${ne.version_count} version${ne.version_count !== 1 ? 's' : ''}<br>
                ${ne.last_updated ? ne.last_updated.slice(0, 10) : ''}
            </span>
        </div>
    `).join('');
}
...
// ── Version List ──────────────────────────────────────────────────────────────
async function selectNE(neName) {
    currentNE = neName;
    selectedForDiff = [];
    ...
    vList.innerHTML = currentVersions.map(v => {
        ...
        <div class="version-card ${isSelected ? 'selected' : ''}" id="vcard-${v.id}">
            ...
            <div class="version-info">
                <div class="v-date">${v.created_at ? v.created_at.slice(0, 16) : ''} &nbsp;·&nbsp; ${sizeKB} KB</div>
                <div class="v-comment">${v.comment || '<em style="color:#bbb">No comment</em>'}</div>
                <div class="v-user">by ${v.uploaded_by_name || 'Unknown'}</div>
            </div>
            <div class="version-actions">
                <button class="btn-select ${isSelected ? 'active-sel' : ''}" onclick="toggleDiffSelect(${v.id})" title="Select for diff">
                    ${isSelected ? '✓' : 'Diff'}
                </button>
                <button class="btn-download" onclick="downloadVersion(${v.id})" title="Download">⬇</button>
                <button class="btn-delete" onclick="deleteVersion(${v.id})" title="Delete">🗑</button>
            </div>
        </div>`;
    }).join('');
}
...
function openDiff() {
    ...
    // Populate dropdowns
    const opts = currentVersions.map(v => `<option value="${v.id}">v${v.version_num} — ${(v.created_at||'').slice(0,10)} — ${v.comment||'no comment'}</option>`).join('');
...
async function uploadVersion() {
    ...
    if (data.success) {
        ...
        cancelUpload();
        await loadNEList();
        if (currentNE === data.ne_name) selectNE(data.ne_name);
    } else {
...
async function deleteVersion(versionId) {
    ...
    if (data.success) {
        await loadNEList();
        if (currentNE) selectNE(currentNE);
    } else {
