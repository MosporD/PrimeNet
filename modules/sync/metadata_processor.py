...
def _pg_safe_double_col(quoted_ident: str) -> str:
    """Postgres-only: coerce numeric-looking text to float; non-numeric → NULL."""
    t = f"NULLIF(TRIM({quoted_ident}), '')"
    # Double braces in the regex so f-strings emit literal ``{0,1}`` quantifiers.
    # Double braces so f-strings emit literal ``{0,1}`` for Postgres quantifiers.
    return (
        f"(CASE WHEN {t} IS NULL THEN NULL "
        f"WHEN {t} ~ '^-{{0,1}}[0-9]+(\\.[0-9]+){{0,1}}([eE]-{{0,1}}[0-9]+){{0,1}}$' "
        f"THEN ({t}::numeric)::double precision ELSE NULL END)"
    )


def _pg_safe_int_col(quoted_ident: str) -> str:
    """Postgres-only: integer PCI/PSC/etc.; '66.0' ok; arbitrary text → NULL."""
    t = f"NULLIF(TRIM({quoted_ident}), '')"
    return (
        f"(CASE WHEN {t} IS NULL THEN NULL "
        f"WHEN {t} ~ '^-{{0,1}}[0-9]+(\\.[0-9]+){{0,1}}$' "
        f"THEN ({t}::numeric)::integer ELSE NULL END)"
    )
...
def _populate_legacy_tables(conn, table_name, technology):
    ...
    sites_sql = f'''
        ...
        {on_site} DO UPDATE SET
            site_name  = COALESCE({ex}.site_name,  sites.site_name),
            latitude   = COALESCE({ex}.latitude,   sites.latitude),
            longitude  = COALESCE({ex}.longitude,  sites.longitude),
            region     = COALESCE({ex}.region,     sites.region),
            vendor     = COALESCE({ex}.vendor,     sites.vendor),
            status     = 'Active',
            updated_at = CURRENT_TIMESTAMP
    '''
    if is_pg:
        execute_query(conn, sites_sql, ())
    else:
        conn.execute(sites_sql)

    # ── Cells ────────────────────────────────────────────────────────────────
    if is_pg:
        execute_query(conn, 'DELETE FROM cells WHERE technology = ?', (technology,))
    else:
    ...
    if is_pg:
        az_sql = _pg_safe_double_col(f'"{az_col}"')
        mtilt_sql = _pg_safe_double_col(f'"{mtilt_col}"')
        etilt_sql = _pg_safe_double_col(f'"{etilt_col}"')
        pci_sql = _pg_safe_int_col(f'"{pci_col}"')
    else:
        az_sql = f"CAST(NULLIF(TRIM(\"{az_col}\"), '') AS {lat_cast})"
        mtilt_sql = f"CAST(NULLIF(TRIM(\"{mtilt_col}\"), '') AS {lat_cast})"
        etilt_sql = f"CAST(NULLIF(TRIM(\"{etilt_col}\"), '') AS {lat_cast})"
        pci_sql = f"CAST(NULLIF(TRIM(\"{pci_col}\"), '') AS INTEGER)"
    ...
    if is_pg:
        ...
        execute_query(conn, adapt_placeholders(cells_sql), ())
    else:
        conn.execute(
            f'''
...
def seed_pm_cells_to_metadata(pm_db_path, vendor):
    """
    For every cell_name in a PM database not yet in metadata,
    ...
    def _site_id(cell_name):
        m = re.match(r'^(\d+)', str(cell_name))
        return m.group(1) if m else None

    def _site_name(cell_name):
        return re.sub(r'[-_][A-Za-z]\d*$', '', str(cell_name))
    ...
    try:
        ...
        if os.path.abspath(pm_db_path) == huawei_abs:
            for tech in PM_TECHNOLOGIES:
                ...
                try:
                    ...
                    for r in rows:
                        cn = r['cell_name'] if isinstance(r, dict) else r[0]
                        pm_rows.append((cn, tech))
                except Exception:
                    continue
        else:
            for tech in PM_TECHNOLOGIES:
                ...
                try:
                    rows = execute_query(
                        pm_conn, f'SELECT DISTINCT cell_name FROM "{table}"', ()
                    ).fetchall()
                    for r in rows:
                        cn = r['cell_name'] if isinstance(r, dict) else r[0]
                        pm_rows.append((cn, tech))
        ...
        existing_cells = {
            r[0] if not isinstance(r, dict) else r['cell_name']
            for r in execute_query(meta_conn, 'SELECT cell_name FROM cells', ()).fetchall()
        }
        existing_sites = {
            r[0] if not isinstance(r, dict) else r['site_id']
            for r in execute_query(meta_conn, 'SELECT site_id FROM sites', ()).fetchall()
        }
        ...
        for cell_name, tech in pm_rows:
            ...
            meta_conn.execute(
                ...
                'VALUES (?, ?, ?, ?, ?)',
                (cell_name, sid, tech, vendor, 'Active'),
            )
        ...
        meta_conn.commit()
        target = METADATA_DB
        logger.info('Seeded %s PM cells from %s into %s.', seeded, pm_db_path, target)
        return seeded
    except Exception as e:
        logger.error('seed_pm_cells_to_metadata failed: %s', e)
        return 0
    finally:
...
def process_metadata_file(file_path, tech, col_map):
    ...
    ex = 'EXCLUDED' if is_pg else 'excluded'
    on_site = 'ON CONFLICT (site_id)' if is_pg else 'ON CONFLICT(site_id)'
    on_cell = 'ON CONFLICT (cell_name)' if is_pg else 'ON CONFLICT(cell_name)'
    upserted = 0
    skipped = 0
    sites_seen = set()
    ...
    for _, row in df.iterrows():
        ...
        if site_id and site_id not in sites_seen:
            site_sql = f'''
                ...
                {on_site} DO UPDATE SET
                    ...
                    latitude   = COALESCE({ex}.latitude,   sites.latitude),
                    longitude  = COALESCE({ex}.longitude,  sites.longitude),
                    site_type  = COALESCE({ex}.site_type,  sites.site_type),
                    vendor     = COALESCE({ex}.vendor,     sites.vendor),
                    status     = 'Active',
                    updated_at = CURRENT_TIMESTAMP
            '''
            execute_query(conn, adapt_placeholders(site_sql), (site_id, site_name, lat, lon, tech, vendor))
        ...
        cell_sql = f'''
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            {on_cell} DO UPDATE SET
                site_id         = COALESCE({ex}.site_id,         cells.site_id),
                technology      = COALESCE({ex}.technology,      cells.technology),
                frequency_band  = COALESCE({ex}.frequency_band,  cells.frequency_band),
                azimuth         = COALESCE({ex}.azimuth,         cells.azimuth),
                mechanical_tilt = COALESCE({ex}.mechanical_tilt, cells.mechanical_tilt),
                electrical_tilt = COALESCE({ex}.electrical_tilt, cells.electrical_tilt),
                vendor          = COALESCE({ex}.vendor,          cells.vendor),
                pci             = COALESCE({ex}.pci,             cells.pci),
                status          = {ex}.status,
                updated_at      = CURRENT_TIMESTAMP
        '''
        execute_query(
            conn,
            adapt_placeholders(cell_sql),
