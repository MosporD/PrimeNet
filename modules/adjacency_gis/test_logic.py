...
@patch('modules.adjacency_gis.logic.load_cells_2g_index', side_effect=_meta_index)
def test_overshoot_flag(_mock_idx):
    sectors = [
        {'dn': 'BTS-A', 'bsc_id': '1', 'segment_name': 'CELL_A', 'cell_name': 'CELL_A',
         'cell_id': 101, 'bcch': 55, 'admin_state': '0'},
    ]
    edges = [
        {'dn': 'ADCE-1', 'source_dn': 'BTS-A', 'adj_ci': 303, 'adj_lac': 1, 'bcch_frequency': 60},
    ]
    payload = build_map_payload(sectors, edges, overshoot_km=5)
    assert payload['edges'][0]['overshoot'] is True
    assert payload['edges'][0]['severity'] in ('overshoot', 'unidirectional', 'co_channel')
