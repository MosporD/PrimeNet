/**
 * Adjacency GIS – Leaflet visualization (Network Map fork, 2G-only).
 * Sites / sector wedges from metadata.db cells_2g until CM NCL pipeline is ready.
 */

// ─── Constants ───────────────────────────────────────────────────────────────

/** This page is the Adjacency GIS copy (body.adjacency-gis-page). */
const ADJACENCY_GIS_MODE = document.body.classList.contains('adjacency-gis-page');
...
const TECH_ORDER = ADJACENCY_GIS_MODE ? ['2G'] : ['2G', '3G', '4G-FDD', '4G-TDD', '5G'];

/** Sort order for cells in the site panel (actual ``technology`` field from API). */
const CELL_TECH_SORT_ORDER = ADJACENCY_GIS_MODE ? ['2G'] : ['2G', '3G', '4G-FDD', '4G-TDD', '5G'];
...
let sectorLayers     = [];
let activeTech       = ADJACENCY_GIS_MODE ? '2G' : 'all';
...
function initializeMap() {
    ...
    // Network Map waits for a tech filter; Adjacency GIS is 2G-only — load sites immediately.
    loadNetworkStats().then(async () => {
...
async function loadNetworkStats() {
    try {
        ...
        if (sitesCountEl) {
            if (ADJACENCY_GIS_MODE) {
                sitesCountEl.textContent = String(Number(s.tech_counts?.['2G']) || 0);
            } else {
...
async function applyNetworkMapState(state /* , opts */) {
    ...
    if (state.activeTech) {
        try { await setTechFilter(ADJACENCY_GIS_MODE ? '2G' : state.activeTech); } catch (_) { /* ignore */ }
    } else if (ADJACENCY_GIS_MODE) {
        try { await setTechFilter('2G'); } catch (_) { /* ignore */ }
    }
