...
def upload_version():
    ...
    _exec(
        cur,
        'SELECT COALESCE(MAX(version_num), 0) + 1 AS next_v FROM config_versions WHERE ne_name = ?',
        (ne_name,),
    )
    row = cur.fetchone()
    version_num = row['next_v'] if isinstance(row, dict) else row[0]

    version_id = _insert_return_id(
        conn,
        '''
...
def diff_versions():
    ...
    conn = get_db()
    cur = conn.cursor()
    _exec(cur, 'SELECT * FROM config_versions WHERE id = ?', (v1_id,))
    v1 = cur.fetchone()
    _exec(cur, 'SELECT * FROM config_versions WHERE id = ?', (v2_id,))
    v2 = cur.fetchone()
    conn.close()
...
def download_version(version_id):
    ...
    conn = get_db()
    cur = conn.cursor()
    _exec(cur, 'SELECT * FROM config_versions WHERE id = ?', (version_id,))
    row = cur.fetchone()
    conn.close()
    if not row:
        return jsonify({'error': 'Version not found'}), 404
...
def delete_version(version_id):
    ...
    conn = get_db()
    cur = conn.cursor()
    _exec(cur, 'DELETE FROM config_versions WHERE id = ?', (version_id,))
    conn.commit()
    conn.close()
