...
def list_sites():
    ...
    rows = execute_query(conn, f'''
        ...
        WHERE {" AND ".join(where)}
        GROUP BY s.site_id, s.site_name, s.latitude, s.longitude, s.vendor
...
def summary():
    try:
        conn = _meta()
        site_count = execute_query(
            conn, "SELECT COUNT(*) AS n FROM sites WHERE status = 'Active'", ()
        ).fetchone()['n']
        cell_count = execute_query(
            conn, "SELECT COUNT(*) AS n FROM cells WHERE status = 'Active'", ()
        ).fetchone()['n']

        tech_rows = execute_query(conn, """
            SELECT technology, COUNT(*) as cnt
            FROM cells WHERE status='Active'
            GROUP BY technology ORDER BY cnt DESC
        """, ()).fetchall()

        vendor_rows = execute_query(conn, """
