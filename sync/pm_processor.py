...
def _coerce_pm_timestamp(raw_ts):
    ...
    # Compact numeric datetime strings
...
def _ensure_pm_cell_timestamp_index_sqlite(conn, table: str) -> None:
    """Index for trend queries: WHERE cell_name = ? AND timestamp >= … ORDER BY timestamp."""
    try:
...
def apply_pm_retention(db_path: str, days: int) -> None:
    """
    Delete PM hourly rows with ``timestamp`` older than ``days`` calendar days.
    Timestamps are normalized to ``YYYY-MM-DD HH:MM:SS`` so lexicographic compare is valid.
    """
    if days <= 0:
        return
    ...
    if use_postgresql():
        from db.runtime import connect_nokia_pm, connect_huawei_pm, execute_query, quote_ident as qi

        abs_db = os.path.abspath(db_path)
        if abs_db == os.path.abspath(NOKIA_PM_DB):
            conn = connect_nokia_pm()
            schema = SCHEMA_NOKIA_PM
        elif abs_db == os.path.abspath(HUAWEI_PM_DB):
            conn = connect_huawei_pm()
            schema = SCHEMA_HUAWEI_PM
        else:
            logger.warning('apply_pm_retention: unknown db_path %s — skipping.', db_path)
            return
        qs = qi(schema)
        try:
            for tbl in tables:
                ...
                cur = execute_query(
                    conn,
                    f'DELETE FROM {qs}.{qt} WHERE timestamp < %s',
                    (cutoff,),
                )
                try:
                    rc = cur.rowcount
            ...
            conn.close()
        logger.info(
            'PM retention PostgreSQL %s: removed rows with timestamp < %s (rowcount sum ≈ %s).',
            db_path,
            cutoff,
            deleted,
        )
    ...
    try:
        ...
        conn.close()
    logger.info(
        'PM retention SQLite %s: removed ~%s rows with timestamp < %s.',
        db_path,
        deleted,
        cutoff,
    )
...
def clear_nokia_pm_tables():
    ...
    if use_postgresql():
        from db.runtime import connect_nokia_pm, quote_ident as qi

        conn = connect_nokia_pm()
        conn.autocommit = True
        cur = conn.cursor()
        for table in tables:
            cur.execute(f'TRUNCATE TABLE {qi(SCHEMA_NOKIA_PM)}.{qi(table)}')
        cur.close()
        conn.close()
        logger.info('Nokia PM: truncated %s table(s) in schema %s.', len(tables), SCHEMA_NOKIA_PM)
        return
...
def _clear_huawei_pm_tables():
    ...
    tables = [pm_table_name(t) for t in PM_TECHNOLOGIES]
    if use_postgresql():
        from db.runtime import connect_huawei_pm, quote_ident as qi

        conn = connect_huawei_pm()
        conn.autocommit = True
        cur = conn.cursor()
        for table in tables:
            cur.execute(f'TRUNCATE TABLE {qi(SCHEMA_HUAWEI_PM)}.{qi(table)}')
        cur.close()
        conn.close()
        logger.info('Huawei PM: truncated %s table(s) in schema %s.', len(tables), SCHEMA_HUAWEI_PM)
        return
...
def huawei_pm_table_for_cell(
    ...
    if use_postgresql():
        from db.runtime import connect_huawei_pm, quote_ident, execute_query

        conn = connect_huawei_pm()
        qs = quote_ident(SCHEMA_HUAWEI_PM)
        for tbl in probe_order:
            qtbl = quote_ident(tbl)
            cur = execute_query(
                conn,
                f'SELECT 1 FROM {qs}.{qtbl} WHERE cell_name = %s LIMIT 1',
                (cell_name,),
            )
            if cur.fetchone():
                conn.close()
