"""
Resolve Nokia raw neighbor export rows (nokia_neighbor_2g/3g/4g_intra/4g_inter) to map line segments.
...
def _is_slim_neighbor_table(raw_table: str, cols: list[str]) -> bool:
    if raw_table == "nokia_neighbor_2g":
        return "source_cell_id" in cols and "target_cell_id" in cols
    if raw_table == "nokia_neighbor_3g":
        return "scid_id" in cols and "tcid_id" in cols
    if raw_table in ("nokia_neighbor_4g", "nokia_neighbor_4g_intra", "nokia_neighbor_4g_inter"):
...
def build_raw_neighbor_lines(
    *,
    ...
    order_hint = f'"{att_col}" DESC' if att_col else "rowid DESC"
    ...
    for row in rows:
        ...
        lines.append(
            {
                "period_start": period,
                "vendor": src_v or (vendor.strip() if (vendor or "").strip() else "") or "Nokia",
