...
from sync_config import HUAWEI_PM_SERVER, HUAWEI_GROUPS_SERVER, PROJECT_ROOT
...
def _convert_excel_file_to_csv(path: str) -> int:
    ...
    if written:
        try:
            ...
            print(f"[csv-normalize] kept source workbook (remove failed): {path} ({e})")
...
def main() -> int:
    raw_root = os.path.join(PROJECT_ROOT, "raw", "huawei")
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        ...
        cells_path = fut_cells.result()
        groups_path = fut_groups.result()
