...
def _extract_xml_from_tgz(tgz_path: Path) -> str:
    ...
    try:
        ...
        raise ValueError(f"invalid or truncated archive: {exc}") from exc
...
def _parse_archive(tgz_path: Path) -> dict | None:
    try:
        ...
        print(f"[warn] skip {tgz_path}: {exc}")
        return None
    if not xml_text.strip():
        print(f"[warn] skip {tgz_path}: no XML payload")
        return None
    try:
...
def main() -> int:
    ...
    deleted_archives = 0
    if _env_bool("FEMTO_DELETE_RAW_AFTER_LOAD", True):
        for tgz in archives:
            try:
                tgz.unlink()
                deleted_archives += 1
                ...
                pass
    elif bad_paths and _env_bool("FEMTO_DELETE_BAD_ARCHIVES", True):
        for tgz in bad_paths:
            try:
                if tgz.is_file() and tgz.stat().st_size == 0:
                    tgz.unlink()
                    print(f"[warn] removed empty archive: {tgz}")
            except OSError:
                pass
    ...
    print(f"[done] raw_archives_deleted={deleted_archives}")
